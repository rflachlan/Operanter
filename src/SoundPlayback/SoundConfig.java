...
public class SoundConfig {
	...
	public LinkedList sourceLines=new LinkedList();
			...
			for (int j=0; j<inf.length; j++){
				...
				if (sdlInfo.matches(inf[j])){
					try{
						...
						int channelCount=1;
						SourceDataLine.Info sdlinfo=(SourceDataLine.Info)inf[j];
						AudioFormat[] formats=sdlinfo.getFormats();
						
						for (AudioFormat f : formats){
							System.out.println(f.getEncoding()+" "+f.getChannels()+" "+f.getFrameRate()+" "+f.getSampleRate()+" "+f.getSampleSizeInBits()+" "+f.isBigEndian());
						}
	...
	public String[] getMixerNames(){
		String[]s=new String[mixerNames.size()];
		for (int i=0; i<mixerNames.size(); i++){
			s[i]=(String)mixerNames.get(i);
		}
		return s;
	}
